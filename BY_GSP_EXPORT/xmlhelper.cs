...
namespace Sanofi_GSP_EXPORT
{
    ...
    public class XmlHelper
    {
        ...
        public DataTable GetNodesData(string XmlPathNode)
        {
            //查找数据。返回一个DataView 
            DataSet ds = new DataSet();
            StringReader read = new StringReader(objXmlDoc.SelectSingleNode(XmlPathNode).OuterXml);
        ...
        public XmlNode GetSingleNode(string XmlPathNode)
        {
            //查找数据。返回一个DataView 
        ...
        /// <param name="AttribContent">属性内容</param>
        /// <param name="Content">元素内容</param>
        public void InsertMutiElement(string MainNode, string Element, Dictionary<String, String> data_dic)
        {
            //插入一个节点，带一属性。 
            XmlNode objNode = objXmlDoc.SelectSingleNode(MainNode);
            XmlElement objElement = objXmlDoc.CreateElement(Element);
            foreach (var dic in data_dic)
            {
        ...
        /// 对xml文件做插入，更新，删除后需做Save()操作，以保存修改
        /// </summary>
        public void SaveAs(string Xml_File_Name)
        {
            ///另存为
            try
            {
                objXmlDoc.Save(Xml_File_Name);
            }
            catch (System.Exception ex)
            {
                throw ex;
